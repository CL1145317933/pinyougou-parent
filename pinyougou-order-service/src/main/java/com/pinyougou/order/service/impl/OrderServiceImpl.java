...
public class OrderServiceImpl implements OrderService {
	...
	public void add(TbOrder order) {
		...
		for (Cart cart : cartList) {
			long orderId = idWorker.nextId();//订单ID的生成
			TbOrder tbOrder = new TbOrder();//重新生成订单对象
			tbOrder.setOrderId(orderId);//订单Id
			tbOrder.setUserId(order.getUserId());//用户名
			tbOrder.setPaymentType(order.getPaymentType());//支付类型
			tbOrder.setStatus("1");//订单状态:未付款
			...
			orderIdList.add(orderId+"");//添加订单ID列表
			total_money+=money;//累加到总金额
		}

		if ("1".equals(order.getPaymentType())){//表示微信支付
			...
			redisTemplate.boundHashOps("payLog").put(payLog.getUserId(),payLog);//存入缓存
		}
	...
	public void updateOrderStatus(String out_trade_no, String transaction_id) {
		...
		payLog.setTransactionId(transaction_id);//流水交易号
		payLogMapper.updateByPrimaryKey(payLog);
		//修改订单状态
		String[] ids = payLog.getOrderList().split(",");//获取订单号数组
